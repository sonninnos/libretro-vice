...
extern int16_t mouse_x2;
extern int16_t mouse_y2;
#endif
    ...
    if (port == JOYPORT_1 || (machine_class == VICE_MACHINE_PLUS4 && port == JOYPORT_PLUS4_SIDCART)) {
        ...
        if (paddles_p1_input == PADDLES_INPUT_JOY_AXIS) {
            ...
            if (_mouse_enabled) {
                ...
                {
                    pot_val[1].x = mouse_paddle_update(pot_val[1].x, &(pot_old[1].x), (int16_t)mouse_x2 / PADDLE_DIV);
                    return (uint8_t)(0xff - pot_val[1].x);
                }
#else
    ...
    if (port == JOYPORT_1 || (machine_class == VICE_MACHINE_PLUS4 && port == JOYPORT_PLUS4_SIDCART)) {
        ...
        if (paddles_p1_input == PADDLES_INPUT_JOY_AXIS) {
            ...
            if (_mouse_enabled) {
                ...
                {
                    pot_val[1].y = mouse_paddle_update(pot_val[1].y, &(pot_old[1].y), (int16_t)mouse_y2 / PADDLE_DIV);
                    return (uint8_t)(0xff - pot_val[1].y);
                }
#else
                /* Use mouse for paddle value */
                pot_val[0].y = mouse_paddle_update(pot_val[0].y, &(pot_old[0].y), (int16_t)mouse_y / PADDLE_DIV);
